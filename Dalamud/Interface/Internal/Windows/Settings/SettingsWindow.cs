...
internal sealed class SettingsWindow : Window
{
    ...
    public override void Draw()
    {
        ...
        using (ImRaii.Disabled(this.CurrentlyOpenTab is SettingsTabAbout))
            ImGui.InputTextWithHint("###searchInput"u8, "搜尋設定項...", ref this.searchInput, 100, ImGuiInputTextFlags.AutoSelectAll);
        ImGui.Spacing();
    ...
    {
        using var tab = ImRaii.TabItem("搜尋結果");
        if (!tab) return;
        ...
        if (!any)
            ImGui.TextColored(ImGuiColors.DalamudGrey, "無搜尋結果...");
    }
